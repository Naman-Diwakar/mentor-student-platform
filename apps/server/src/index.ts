...
if (!supabaseUrl || !supabaseAnonKey || !supabaseServiceRoleKey) {
  throw new Error(
    "Missing Supabase server environment variables. Check NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, and SUPABASE_SERVICE_ROLE_KEY."
  );
}
...
const roomStates = new Map<string, EditorState>();
const roomMessages = new Map<string, ChatMessage[]>();
const roomDocs = new Map<string, Y.Doc>();
const roomAwareness = new Map<
  string,
  Map<string, { socketId: string; clientIds: number[]; update: number[] }>
>();
const roomMediaReady = new Map<string, Set<string>>();
...
function getRoomName(sessionId: string, channel: "session" | "collab" = "session") {
  return `session:${sessionId}:${channel}`;
}

function getOrCreateRoomDoc(sessionId: string) {
...
function syncRoomStateFromDoc(sessionId: string) {
...
async function getSessionById(sessionId: string) {
  const { data, error } = await supabaseAdmin
    .from("sessions")
    .select(
      "id, title, status, invite_code, mentor_id, student_id, started_at, ended_at, created_at"
    )
    .eq("id", sessionId)
    .single();
...
async function endSessionByMentor(sessionId: string, mentorId: string) {
...
function canAccessSession(session: SessionRecord, userId: string) {
...
app.get(
  ...
  async (request: AuthenticatedRequest, response: Response) => {
    ...
    const sessionColumn =
      profile.role === "mentor" ? "mentor_id" : "student_id";
...
io.use(async (socket, next) => {
  const token = getSingleValue(
    socket.handshake.auth.token as string | string[] | undefined
  );
  const sessionId = getSingleValue(
    socket.handshake.auth.sessionId as string | string[] | undefined
  );
  if (!token || !sessionId) {
...
io.on("connection", (socket) => {
  ...
  const profile = socket.data.profile as Profile | null;
  ...
  socket.emit(
    "collab:awareness-history",
    Array.from(roomAwareness.get(sessionId)?.values() ?? [])
  );
  if (profile) {
  ...
  socket.on("editor:update", (payload: Partial<EditorState>) => {
    const currentState = roomStates.get(sessionId) ?? DEFAULT_EDITOR_STATE;
  ...
  socket.on("collab:update", (payload: number[] | Uint8Array) => {
  ...
  socket.on(
    ...
    (payload: { clientIds?: number[]; update?: number[] | Uint8Array }) => {
